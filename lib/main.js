...
var LAST_BUILD_ENV_VARS = '%s/job/%s/lastBuild/injectedEnvVars' + API;
...
var init = exports.init = function(host, options) {
    ...
    return {
        ...
        last_build_env_vars: function(jobname, callback) {
            /*
            Get information for the environment variables from the last build
            */
            request({method: 'GET', url: build_url(LAST_BUILD_ENV_VARS, jobname)}, function(error, response, body) {
                if ( error || response.statusCode !== 200 ) {
                    callback(error || true, response);
                    return;
                }
                var data = JSON.parse(body.toString());
                callback(null, data.envMap);
            });
        }
    }
};
